...
/**
 * Creates a prompt to instruct the AI to provide a concise, plain-text explanation
 * of a specific modification step within the broader project context.
 * The explanation should clarify what the AI is about to do for the given file,
 * why it's doing it, and how it fits into the overall plan.
 *
 * @param projectContext The broader project context string.
 * @param stepFilePath The path of the file being modified or created in this step.
 * @param stepInstruction The specific instruction for this step (e.g., `generate_prompt` or `modification_prompt`).
 * @param existingFileContent The current content of the file if it's an existing file being modified, or `undefined` for a new file.
 * @param relevantSnippets Relevant code snippets from other files for additional context.
 * @param chatHistory The recent chat history for conversational context.
 * @param recentChanges A formatted string detailing changes made during the current workflow execution.
 * @returns A string prompt for the AI.
 */
export function createPerStepExplanationPrompt(
	projectContext: string,
	stepFilePath: string,
	...
	const existingContentSection = existingFileContent
		? `
	...
	const recentChangesSection = recentChanges
		? `
    --- Recent Changes (during this workflow) ---
    ${recentChanges}
    --- End Recent Changes ---`
		: "";

	return `You are an expert software engineer. Your task is to provide a comprehensive explanation of the next action you are about to take. Make sure the entire explanation is readable.
    ...
    - SUMMARY of the entire current step.
    ...
    *   **Provide Rationale (Why):** Clearly explain **why** this action is being taken, referencing the overall plan, the specific instruction for this step (\`${stepInstruction}\`), and its contribution to the larger goal.
    *   **Detail Implementation (How):** Outline **how** the AI intends to implement the change for this specific step, including any specific considerations for code quality, modularity, best practices, and integration with existing structures (e.g., "This modification will involve adding a new utility function to \`utils.ts\`, ensuring it handles edge cases and follows existing TypeScript conventions for type safety.").
    *   **Utilize Markdown Formatting:** Explicitly instruct the AI to use Markdown formatting (e.g., bullet points for sub-tasks, inline code snippets \`like this\`, bold text for emphasis) within its explanation to enhance clarity and readability, mirroring the clarity standards of the main plan.
    *   **Focus on the Current Step:** Reiterate that the explanation should *only* pertain to the immediate step about to be executed, not the entire remaining plan.
    *   **Maintain Production-Ready Standards:** Remind the AI to explain the code quality aspects (robustness, maintainability, security, clean code principles, modularity) for the *code related to this specific step*, if applicable.
    ...
    --- File Target ---
    File Path: ${stepFilePath}
    Instruction for this step: ${stepInstruction}
    ...
    --- End Recent Changes Section ---

    Your comprehensive explanation (using Markdown for clarity and detail, no surrounding conversational fluff, JSON, or markdown code blocks):`;
}
